...
configure_dialog () {
 ...
 if [[ -n $(grep Sailfish <<< $DetectedOS) ]]; then		# Sailfish OS mod config table. read -i parameter missing in Sailfish OS => shell config dialog disabled
	...
	read -e -p " Enter exluded FS (e.g. snap ...): " 					-i "${FSExclude:-snap}"  					FSExclude
	...
	read -e -p " Max length of device & mountpoint (default 30): " 				-i "${MaxStringLength:-30}" 					MaxStringLength
	...
	for i in $ConfigParameterList MaxStringLength; do
...
load_processing_vars () {
	...
	# Defaultwert falls nicht in Config
	: ${MaxStringLength:=30}

	ColumnHeader="System-Device FS-Type Size Used Avail Used% Mountpoint Used-Graph"
	SeperatorLine=$(echo $FrameColor"----------------------------------------------------------------------------------------------------------$ResetColor")
	FSExclude=$( sed 's/ /\\|/g' <<< "$FSExclude")		# sed / grep parser

	# Hilfsfunktion: erstes UND letztes Feld auf max Länge kürzen (ohne ...)
	shorten_first_and_last() {
		awk -F " " -v max="$MaxStringLength" '{
			if (length($1) > max) $1 = substr($1,1,max);
			nf = NF;
			if (length($nf) > max) $nf = substr($nf,1,max);
			print $0
		}'
	}
	...
	FSLocalSystemList=$(df -hTP | sed '1,1d'| grep -vw "$FSExclude" 2>/dev/null | \
			egrep -w $(echo $FSLocalSystems| tr " " "|") | shorten_first_and_last | \
	...
	FSLocalStorageList=$(df -hTP | sed '1,1d' | grep -v localhost | grep -vw "$FSExclude" 2>/dev/null | \
			egrep    $(echo $FSLocalStorage| tr " " "|") | shorten_first_and_last | \
	...
	FSRemoveableList=$(df -hTP | sed '1,1d' | grep -v tmpfs | grep -vw "$FSExclude" 2>/dev/null | \
			...
			shorten_first_and_last | awk -F " " '{print $1,$2,$3,$4,$5,$6,$7,$8,$9}' | sort -u -k$SortFSColumnStorage)

	FSRemoteList=$(df -hTP | sed '1,1d' | grep -vw "$FSExclude" 2>/dev/null | \
			egrep -w $(echo $FSRemote| tr " " "|")  | sed 's/:/ _/g' | shorten_first_and_last | \
			awk -F " " '{print $1,$3,$4,$5,$6,$7,$8,$9}' | sort -u -k$SortFSColumnRemote )
}
#-------------------------------------------------------------------------------------------------------------------------------------------------------
check_for_LAN_host () {
...
# changelog
#
# 2.3.1  add MaxStringLength var for Device und Mountpoint
