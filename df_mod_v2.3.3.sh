...
configure_dialog () {
	...
	if [[ -n $(grep Sailfish <<< $DetectedOS) ]]; then
		...
		read -e -p " Max length device column (default 40): " -i "${MaxDeviceLength:-40}" MaxDeviceLength
		read -e -p " Max length mountpoint column (default 60): " -i "${MaxMountpointLength:-60}" MaxMountpointLength
		...
		read -e -p " Enter max. stringlenght for devicename (digit|dyn): " -i "${MaxDeviceLength:-dyn}" MaxDeviceLength
		read -e -p " Enter max. stringlenght for mountpoint (digit|dyn): " -i "${MaxMountpointLength:-dyn}" MaxMountpointLength
...
load_processing_vars () {
    ...
    #configure_max_string_lenght
    # set default vars for missing config vars
    : ${MaxDeviceLength:=40}
    : ${MaxMountpointLength:=60}

    if [[ ! "$MaxDeviceLength" =~ ^[0-9]+$ ]]; then MaxDeviceLength=$(df | awk '{print length($1)}' | sort -nr | head -1); fi
    if [[ ! "$MaxMountpointLength" =~ ^[0-9]+$ ]]; then MaxMountpointLength=$(df | awk '{print length($NF)}' | sort -nr | head -1); fi

    # Helper function: Shortens only Device (Column 1) and Mountpoint (Column 6)
    shorten_device_and_mountpoint() {
        awk -F " " -v dev_max="$MaxDeviceLength" -v mp_max="$MaxMountpointLength" '{
            if (length($1) > dev_max) $1 = substr($1,1,dev_max);
            if (length($7) > mp_max) $7 = substr($7,1,mp_max);
            print $0
        }'
    }

    FSLocalSystemList=$(df -hTP | sed '1,1d'| grep -vw "$FSExclude" 2>/dev/null | \
            egrep -w $(echo $FSLocalSystems| tr " " "|") | shorten_device_and_mountpoint | \
    ...
    FSLocalStorageList=$(df -hTP | sed '1,1d' | grep -v localhost | grep -vw "$FSExclude" 2>/dev/null | \
            egrep $(echo $FSLocalStorage| tr " " "|") | shorten_device_and_mountpoint | \
    ...
    FSRemoveableList=$(df -hTP | sed '1,1d' | grep -v tmpfs | grep -vw "$FSExclude" 2>/dev/null | \
            ...
            shorten_device_and_mountpoint | awk -F " " '{print $1,$2,$3,$4,$5,$6,$7,$8,$9}' | sort -u -k$SortFSColumnStorage)

    # Korrigierte Zeile für Remote (SSHFS) – ALLE 9 Felder behalten!
    FSRemoteList=$(df -hTP | sed '1,1d' | grep -vw "$FSExclude" 2>/dev/null | \
            egrep -w $(echo $FSRemote| tr " " "|") | shorten_device_and_mountpoint | \
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------
print_parser_list () {
	echo "$@" | awk -F " " '{printf " %-'${MaxDeviceLength}'s %10s %9s  %6s   %6s   %6s    %11s   %-20s \n", $1, $2, $3, $4, $5, $6, $8, $7}'
}
#-------------------------------------------------------------------------------------------------------------------------------------------------------
print_graph_star () {
...
# 2.3.2  add MaxDeviceLength= + MaxMountpointLength= fixed
