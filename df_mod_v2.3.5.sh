...
configure_dialog () {
	# create config file
	ConfigParameterList=$(cat $0 | grep -A74 "configure_dialog () {" | grep "read -e -p \" Enter" | awk -F " " '{print $NF}')
	...
	if [[ -n $(grep Sailfish <<< $DetectedOS) ]]; then
		...
		read -e -p " Enter Max length outputwidth (digit|dynamic): " -i "${MaxOutputWidth:-dyn}" MaxOutputWidth
...
load_processing_vars () {
    ...
    # configure max string and seperatorline/output lenght/width, var=digit use digit, else calculate digit
    ...
    if [[ ! "$MaxOutputWidth" =~ ^[0-9]+$ ]]; then MaxOutputWidth=$(tput cols); fi

    ColumnHeader="System-Device FS-Type Size Used Avail Used% Mountpoint Used-Graph"
    SeperatorLine="${FrameColor}$(printf '%*s' "$MaxOutputWidth" | tr ' ' '-')${ResetColor}"
